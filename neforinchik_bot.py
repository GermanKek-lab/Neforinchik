...
def start(message):
    ...
    bot.send_message(message.chat.id, f'Привет, {message.from_user.first_name}! '
                                      ...
                                      f'ТЫ кто такой по жизни??', reply_markup=markup)

@bot.message_handler(content_types=['text'])
def bot_message(message):
    main_markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    main_markup.add(types.KeyboardButton('Создать анкету'), types.KeyboardButton('СМотреть анкеты'),
                    types.KeyboardButton('Редактировать анкету'), types.KeyboardButton('Смотреть мемы с нефорами'),
                    types.KeyboardButton('Контакты форов'), types.KeyboardButton('Контакты рефоров'))
